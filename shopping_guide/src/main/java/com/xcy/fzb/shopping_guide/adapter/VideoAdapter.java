...
//                FinalContents.showShare(rows.get(position).getTitle(),"http://test.fangzuobiao.com:88/VideoSharing?"+"&userId="+FinalContents.getUserID()+"&talkToolId="+rows.get(position).getId(),rows.get(position).getContent(),"http://39.98.173.250:8080"+rows.get(position).getImg(),"http://test.fangzuobiao.com:88/sellingPoint?"+"&userId="+FinalContents.getUserID()+"&talkToolId="+rows.get(position).getId(),context);
            }
        });
    }
